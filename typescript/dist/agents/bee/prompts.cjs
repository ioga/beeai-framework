...
const BeeSystemPrompt = new template_cjs.PromptTemplate({
  schema: zod.z.object({
    ...
    createdAt: zod.z.string().datetime().nullish()
  }),
...
const BeeAssistantPrompt = new template_cjs.PromptTemplate({
  schema: zod.z.object({
    ...
    toolInput: zod.z.array(zod.z.string()),
    toolOutput: zod.z.array(zod.z.string()),
    finalAnswer: zod.z.array(zod.z.string())
  }).partial(),
...
const BeeUserPrompt = new template_cjs.PromptTemplate({
  ...
  functions: {
    formatMeta: /* @__PURE__ */ __name(function() {
      const meta = this.meta;
      if (!meta) {
        return "";
      }
      const parts = [
        meta.createdAt && `This message was created at ${meta.createdAt}`
      ].filter(Boolean).join("\n");
      return parts ? `
