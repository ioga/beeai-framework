...
import { BeeSystemPrompt, BeeAssistantPrompt, BeeUserPrompt, BeeSchemaErrorPrompt, BeeToolNotFoundPrompt, BeeToolErrorPrompt, BeeToolInputErrorPrompt, BeeUserEmptyPrompt, BeeToolNoResultsPrompt } from '../../prompts.js';
...
import { z } from 'zod';
import { Role, UserMessage, AssistantMessage, SystemMessage } from '../../../../backend/message.js';
...
class DefaultRunner extends BaseRunner {
  static {
  ...
  async llm({ signal, meta, emitter }) {
    ...
    return new Retryable({
      ...
      onError: /* @__PURE__ */ __name(async (error) => {
        ...
        if (error instanceof LinePrefixParserError) {
          if (error.reason === LinePrefixParserError.Reason.NoDataReceived) {
            await this.memory.add(new AssistantMessage("\n", {
              [tempMessageKey]: true
            }));
          } else {
            await this.memory.add(new UserMessage(this.templates.schemaError.render({}), {
              [tempMessageKey]: true
            }));
          }
        }
      ...
      executor: /* @__PURE__ */ __name(async () => {
        ...
        await parser.end();
        await this.memory.deleteMany(this.memory.messages.filter((msg) => getProp(msg.meta, [
          tempMessageKey
        ]) === true));
        return {
  ...
  async tool({ state, signal, meta, emitter }) {
    ...
    if (!tool) {
      this.failedAttemptsCounter.use(new AgentError(`Agent was trying to use non-existing tool "${state.tool_name}"`, [], {
        context: {
          state,
          meta
        }
  ...
  get renderers() {
    const self = {
      user: {
        message: /* @__PURE__ */ __name(({ prompt }) => prompt !== null || this.input.memory.isEmpty() ? new UserMessage(prompt || this.templates.userEmpty.render({})) : void 0, "message")
      },
      system: {
        variables: {
        ...
        message: /* @__PURE__ */ __name(async () => new SystemMessage(this.templates.system.render({
  ...
  async initMemory({ prompt }) {
    ...
    const prevConversation = [
      ...
      if (message.role === Role.USER) {
        ...
        const text = isEmpty ? (this.templates?.userEmpty ?? BeeUserEmptyPrompt).render({}) : (this.templates?.user ?? BeeUserPrompt).render({
          input: message.text,
          meta: {
            ...message?.meta,
    ...
    const memory = new TokenMemory({
      ...
      handlers: {
        removalSelector(curMessages) {
          ...
          const lastMessage = curMessages.length > 3 ? curMessages.find((msg) => msg.role === Role.ASSISTANT && getProp(msg, [
            "ctx",
            "success"
          ]) === false) ?? curMessages.find((msg) => msg.role === Role.ASSISTANT) : null;
          if (!lastMessage) {
  ...
  createParser(tools) {
    const parser = new LinePrefixParser({
      ...
      tool_name: {
        ...
        field: new ZodParserField(z.pipeline(z.string().trim(), z.enum(tools.map((tool) => tool.name))))
      },
      tool_input: {
