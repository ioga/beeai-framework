...
const GraniteBeeAssistantPrompt = BeeAssistantPrompt.fork((config) => {
...
const GraniteBeeSystemPrompt = BeeSystemPrompt.fork((config) => {
...
const GraniteBeeSchemaErrorPrompt = BeeSchemaErrorPrompt.fork((config) => {
...
const GraniteBeeUserPrompt = BeeUserPrompt.fork((config) => {
  config.template = `{{input}}`;
});
const GraniteBeeToolNotFoundPrompt = BeeToolNotFoundPrompt.fork((config) => {
  config.template = `Tool does not exist!
{{#tools.length}}
Use one of the following tools: {{#trim}}{{#tools}}{{name}},{{/tools}}{{/trim}}
{{/tools.length}}`;
});
const GraniteBeeToolErrorPrompt = BeeToolErrorPrompt.fork((config) => {
...
const GraniteBeeToolInputErrorPrompt = BeeToolInputErrorPrompt.fork((config) => {
