...
async function callCodeInterpreter({ url, body, signal }) {
  const response = await fetch(url, {
    ...
    if (error.cause.name == "HTTPParserError") {
      throw new base_cjs.ToolError("Request to bee-code-interpreter has failed -- ensure that CODE_INTERPRETER_URL points to the new HTTP endpoint (default port: 50081).", [
        error
      ]);
    } else {
      throw new base_cjs.ToolError("Request to bee-code-interpreter has failed.", [
        error
      ]);
    }
  });
  if (!response?.ok && response.status > 400) {
    throw new base_cjs.ToolError(`Request to bee-code-interpreter has failed with HTTP status code ${response.status}.`, [
