...
var pThrottle__default = /*#__PURE__*/_interopDefault(pThrottle);
...
class DuckDuckGoSearchTool extends base_cjs$1.Tool {
  ...
  _createClient() {
    ...
    return throttle === false ? duckDuckScrape.search : pThrottle__default.default({
      ...throttle,
  ...
  async _run({ query: input }, options, run) {
    ...
    return new DuckDuckGoSearchToolOutput(results.map((result) => ({
      title: stringStripHtml.stripHtml(result.title).result,
      description: stringStripHtml.stripHtml(result.description).result,
      url: result.url
    })));
  }
  loadSnapshot(snapshot) {
