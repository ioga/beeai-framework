...
class LangChainChatModel extends chat_cjs.ChatModel {
  static {
  ...
  prepareOutput(output) {
    ...
    if (typeof output.content === "string") {
      messages.push(new message_cjs.AssistantMessage(output.content));
    } else {
      messages.push(new message_cjs.AssistantMessage(output.content.map((message) => {
        if (message.type === "text") {
          ...
          throw new errors_cjs.ValueError(`Unknown message type "${message.type}"`);
        }
      })));
    }
    const usage = {
  ...
  async _createStructure(input, run) {
    ...
    const object = this.lcLLM.withStructuredOutput(input.schema, {
      method: "jsonSchema",
      strict: false
