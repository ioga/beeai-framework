...
class VercelChatModel extends chat_cjs.ChatModel {
  ...
  transformMessages(messages) {
    return messages.flatMap((msg) => {
      if (msg.role === "tool") {
        return new message_cjs.ToolMessage(msg.content, msg.experimental_providerMetadata);
      }
      return new message_cjs.AssistantMessage(msg.content, msg.experimental_providerMetadata);
    });
  }
  createSnapshot() {
    return {
